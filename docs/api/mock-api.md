...
| `title` | String | Yes | Autogenerated descriptive title for the property |
| `monthlyRent` | int | Yes | Monthly rent (SGD) |
| `flat_id` | String | Yes | Foreign key used to match this listing to the offline transit matrix |
| `address` | String | No | Real block and street name |
| `roomType` | String | No | Category of room (e.g., "Condo room", "HDB room") |
| `sourcePlatform` | String | No | Source site, e.g., `PropertyGuru`, `99.co` |
| `amenities` | JSON String | Yes | A nested JSON string containing boolean flags (e.g., `hasAircon`, `hasGym`) |
| `destinationTags` | List<String> | No | Optional tags indicating which destinations this listing set was curated around |
| `notes` | String | No | Additional curated notes |
